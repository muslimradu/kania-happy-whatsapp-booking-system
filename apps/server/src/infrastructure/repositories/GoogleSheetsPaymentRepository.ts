 ...
 * A=0 invoice_number | B=1 booking_id | C=2 amount | D=3 method
  ...
  implements IPaymentRepository
{
  ...
  async updateStatus(
    ...
    if (meta?.proofImageUrl) {
      await this.updateCell(rowIndex, COL.PROOF_IMAGE_URL, meta.proofImageUrl);
    }
    if (meta?.verifiedBy) {
  ...
  private toEntity(row: string[]): Payment {
    return {
      ...
      method:         this.safeCell(row, COL.METHOD) as PaymentMethod,
